...
# =====================================================================
IDENTITIES: List[str]          = ["A", "B", "C", "D"]
ACTIVITY_TOPICS: List[str]     = ["V1", "V2"]
DIRICHLET_PARAMS: List[float]  = [8, 8, 8]
...
DE_LOGNORM_MEAN: float  = 3
DE_LOGNORM_SIGMA: float = 0.4

OUT_DIR_DEFAULT: Path          = Path("../data/ABCD_V1V2/")
...
def simulate_counts(
    *,
    ...
    return counts_df, lib_df, gm, theta_df, gene_meta
...
def main():
    ...
    counts_df.to_csv(args.out / "counts.csv")
    lib_df.to_csv(args.out / "library_sizes.csv")
    gm.to_csv(args.out / "gene_means.csv")
    theta_df.to_csv(args.out / "theta.csv")
    gene_meta.to_csv(args.out / "gene_metadata.csv")
