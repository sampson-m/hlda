...
def initialize_long_data(count_matrix, cell_identities, n_fc_nodes, save_path=None):
    ...
    topic_hierarchy=create_topic_hierarchy(unique_identities, n_fc_nodes)
...
def run_gibbs_sampling_numba(long_data, constants, hyperparams, num_loops, burn_in, output_dir, identity_mapping):
    ...
    n_fc_nodes = len(B) - num_identities

    cell_ids = np.unique(long_data['cell_identity'])
    unique_cell_ids = [identity_mapping[t] for t in cell_ids]
    topic_hierarchy = create_topic_hierarchy(unique_cell_ids, n_fc_nodes)
...
def run_gibbs_pipeline(n_fc_nodes, num_loops, burn_in, hyperparams, sample_dir, est_dir, input_file):
    ...
    constants = compute_constants(long_data, n_fc_nodes)
